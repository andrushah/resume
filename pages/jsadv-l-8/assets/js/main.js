...
let wwTrext = document.getElementById('wwTrext'); //Доступ до поля вводу нових слів
let wrongWords = document.querySelector('.wrong-words'); // Достуд до елемента куди будуть записуватись нові слова
let textArea = document.querySelector('#textarea'); // Доступ до текстового поля що буде фільтруватися
let addBtn = document.querySelector('#addBtn'); // Доступ до кнопки добавити
let resetBtn = document.querySelector('#resetBtn'); // Кнопка RESET
let censorBtn = document.querySelector('#censoreBtn'); // Кнопка Censor
let ivanidField = false; // змінна для валідації 
let ivanidTextArea = false; // змінна для валідації 
...
wwTrext.addEventListener('input', function () {
    if (ivanidField) {
        this.value.length < 1 ? this.classList.add('invalide') : this.classList.remove('invalide');
        ivanidField = false;
    }
});
// валідація поля на подію ONINPUT
