...
// перевірка правильності результату
    $('#check').on('click', function () {
        ...
        // якщо к-сть ел. з права відповідає потрібній кількості і перевірка покищо не робилась тоді робимо превірку
    ...
    $('#left>li').draggable({
        ...
        start: function () {
            checked = 0; // скидання результату якщо перед стартом не було натиснуто кнопку стар
            $('#checkResult').attr('disabled', false);
            ...
            bg = $(this).css('background'); // передачаа власної властивості в глобальну змінну
            textContent = $(this).text(); // передачаа власної властивості в глобальну змінну
            /** тобто при drag ми лише записали потрібні дані в змінні які потім при drop запишемо в наведений елемент */
        }
    });
    ...
    $('#right>li').droppable({
        ...
        // у функцію передаємо клітинку в яку будем передавати значення властивостей, а також елемен який передає властивості
        drop: function (event, ui) {
            // при дропі якщо фон не задано тоді
            if ($(this).css('background').includes("none")) {
                // значиті клітинка порожня встановлюєм нові властивості
                ui.draggable[0].style.opacity = '0'; // блокуєм елемент(з лівої частини) який передав властивість
